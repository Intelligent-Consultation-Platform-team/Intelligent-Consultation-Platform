...
const loadSession = async () => {
  ...
  try {
    const detail = await aiConsultationApi.getSessionDetail(sessionId.value)
    const data = detail?.data || detail
    riskLevel.value = data?.riskLevel || 'medium'
    status.value = data?.status || 'active'
    suggestion.value = data?.summary || suggestion.value

    const list = await aiConsultationApi.getSessionMessages(sessionId.value, { page: 1, pageSize: 50 })
...
const handleSendMessage = async (content) => {
  ...
  sending.value = true
  try {
    const res = await aiConsultationApi.sendMessage({ sessionId: sessionId.value, content, contentType: 'text' })
...
const handleCloseSession = async () => {
  try {
    await aiConsultationApi.closeSession(sessionId.value, { closeReason: '用户主动结束问诊' })
    status.value = 'closed'
...
const handleFileChange = async (file) => {
  ...
  try {
    await aiConsultationApi.uploadFile(formData)
    ElMessage.success('文件已上传，后续可接入预处理识别流程')
  } catch (error) {
